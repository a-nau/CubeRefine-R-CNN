...
def get_cfg_defaults(cfg):
    ...
    cfg.MODEL.CUBE_ON = True
    cfg.MODEL.MESH_ON = True
    cfg.MODEL.FREEZE_BACKBONE_COMPLETE = False
    cfg.MODEL.FREEZE_RPN = False
    cfg.MODEL.FREEZE_BOX_HEAD = False
    cfg.MODEL.FREEZE_CUBE_HEAD = False
    cfg.MODEL.FREEZE_MESH_HEAD = False

    cfg.SOLVER.LR_ROP_PATIENCE = 200
    cfg.SOLVER.LR_ROP_FACTOR = 0.1
    ...
    cfg.MODEL.ROI_MESH_HEAD.POOLER_SAMPLING_RATIO = 2
    # Type of pooling operation applied to the incoming feature map for each RoI
    cfg.MODEL.ROI_MESH_HEAD.POOLER_TYPE = "ROIAlign"
    # Numer of stages
    cfg.MODEL.ROI_MESH_HEAD.NUM_STAGES = 3
    cfg.MODEL.ROI_MESH_HEAD.NUM_GRAPH_CONVS = 3  # per stage
    cfg.MODEL.ROI_MESH_HEAD.GRAPH_CONV_DIM = 128
    ...
    cfg.MODEL.ROI_MESH_HEAD.NORMALS_LOSS_WEIGHT = 0.1
    cfg.MODEL.ROI_MESH_HEAD.EDGE_LOSS_WEIGHT = 1.0
    # coord thresh
    cfg.MODEL.ROI_MESH_HEAD.GT_COORD_THRESH = 0.0  # 5.0
    # Init ico_sphere level (only for when voxel_on is false)
    cfg.MODEL.ROI_MESH_HEAD.ICO_SPHERE_LEVEL = -1
    cfg.MODEL.ROI_MESH_HEAD.CUBOID_MESH_DIVISION_LEVELS = 4

    # These 2 keys are for the network architecture modification using residual GCN and 0N-GCN.
    # ZN_GCONV: an int variable represents the ratio of vertex features that influence other vertices.
    cfg.MODEL.ROI_MESH_HEAD.ZN_GCONV = 0.0
    cfg.MODEL.ROI_MESH_HEAD.RESIDUAL = False  # * legacy, not used anymore!
    cfg.MODEL.ROI_MESH_HEAD.CONV_TYPE = "Graph"
