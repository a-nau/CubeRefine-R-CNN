...
def print_ap_category_histogram(dataset, results):
    ...
    table = tabulate(
        data,
        headers=["category", "AP2D", "AP3D", "MeshAP50"] * (N_COLS // 2),
        tablefmt="pipe",
        numalign="left",
        stralign="center",
    )
...
def print_ap_omni_histogram(results):
    ...
    logger.info(
        "Omni3D performance on test set. The numbers below should be used to compare to others approaches on Omni3D, such as Cube R-CNN"
    )
