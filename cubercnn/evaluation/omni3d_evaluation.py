...
from cubercnn.structures.mesh import (
    DEFAULT_BBOX3D_FACES,
    normalize_mesh_from_verts_and_faces,
)
...
class Omni3DEvaluator(COCOEvaluator):
    def __init__(
        ...
        *,
        ...
        self._normalize3d = "real" in dataset_name
    ...
    def _derive_omni_results(self, omni_eval, iou_type, mode, class_names=None):
        ...
        metrics = {
            ...
            "3D": ["AP", "AP15", "AP25", "AP50", "AP75", "AP95", "APn", "APm", "APf"],
        }[mode]
    ...
    def _eval_predictions(self, predictions, img_ids=None):
        ...
        for task in sorted(tasks):
            ...
            for mode in modes:
                ...
                self._results[task + "_" + format(mode) + "_evalImgs"] = evals[
                    mode
                ].evalImgs
                self._results[
                    task + "_" + format(mode) + "_evals_per_cat_area"
                ] = evals[mode].evals_per_cat_area
...
def instances_to_coco_json(instances, img_id):
    ...
    if hasattr(instances, "pred_bbox3D"):
        bbox3D = instances.pred_bbox3D.tolist()  # vertices only
        bbox3D_faces = instances.pred_bbox3D_faces.tolist()
        ...
        bbox3D_faces = np.ones([num_instances, 12, 3]).tolist()
    ...
    if hasattr(instances, "pred_meshes"):
        mesh_verts = instances.pred_meshes.verts_list()
        mesh_faces = instances.pred_meshes.faces_list()
        mesh_verts_world = instances.pred_meshes_world.verts_list()
        mesh_faces_world = instances.pred_meshes_world.faces_list()
    else:
        # todo: dummy values don't make sense
        mesh_verts = np.zeros([num_instances, 8, 3])
        mesh_faces = np.zeros([num_instances, 8, 3])
        mesh_verts_world = np.zeros([num_instances, 8, 3])
        mesh_faces_world = np.zeros([num_instances, 8, 3])
    ...
    for k in range(num_instances):
        result = {
            ...
            "bbox3D": bbox3D[k],
            "bbox3D_faces": bbox3D_faces[k],
            "center_cam": center_cam[k],
            "center_2D": center_2D[k],
            "dimensions": dimensions[k],
            "pose": pose[k],
            "mesh_verts": mesh_verts[k].tolist(),
            "mesh_faces": mesh_faces[k].tolist(),
            "mesh_verts_world": mesh_verts_world[k].tolist(),
            "mesh_faces_world": mesh_faces_world[k].tolist(),
        }
...
class OmniEval(COCOeval):
    ...
    def evaluate(self):
        ...
        self.ious = {
            (imgId, catId): self.computeIoU(imgId, catId, normalize=self.normalize3d)
            for imgId in p.imgIds
            for catId in catIds
        }

        if False and self.mode == "3D":
            self.shape_metrics = {
                (imgId, catId): self.compute_mesh_metrics(imgId, catId)
                for imgId in p.imgIds
                for catId in catIds
            }
            print(self.shape_metrics)
    ...
    def computeIoU(self, imgId, catId, normalize=False):
        ...
        if p.iouType == "bbox":
            if self.mode == "2D":
                ...
                # * Note: since we converted data to Mitsuba CS (for Mesh Head) we need to convert back here!
                cs_transform = Rotation.from_matrix([[-1, 0, 0], [0, -1, 0], [0, 0, 1]])
                d = [cs_transform.apply(d["bbox3D"]).tolist() for d in dt]
        else:
        ...
        if self.mode == "2D":
            ...
            device = (
                ...
                and len(d) * len(g) < MAX_DTS_CROSS_GTS_FOR_IOU3D
            ...
            if normalize:
                d = [
                    normalize_mesh_from_verts_and_faces(
                        torch.tensor(i), faces=DEFAULT_BBOX3D_FACES
                    ).verts_list()[0]
                    for i in d
                ]
                g = [
                ...
                dd = torch.stack(d).to(device).float()
                gg = torch.stack(g).to(device).float()
            else:
    ...
    def evaluateImg(self, imgId, catId, aRng, maxDet):
        ...
        T = len(p.iouThrs)  # for each IoU treshold
        G = len(gt)
        D = len(dt)
        gtm = np.zeros((T, G))  # for each IoU threshold, check if match
        dtm = np.zeros((T, D))  # we have more detections than GT, thus, check all
