...
class RPNWithIgnore(RPN):
    ...
    def label_and_sample_anchors(
        ...
        for gt_boxes_i, gt_boxes_ign_i in zip(gt_boxes, gt_boxes_ign):
            ...
            best_inds = torch.tensor(
                list(
                    set(best_ious_gt_ind.tolist())
                    & set((gt_labels_i == 1).nonzero().squeeze(1).tolist())
                )
            )
            ...
            if len(gt_boxes_ign_i) > 0:
                ...
                if background_inds.numel() > 1:
                    ...
                    match_quality_matrix_ign = retry_if_cuda_oom(pairwise_ioa)(
                        gt_boxes_ign_i, anchors[background_inds]
                    )
    ...
    def losses(
        ...
        if not self.objectness_uncertainty.lower() in ["none"]:
            (
                localization_loss,
                objectness_loss,
            ) = _dense_box_regression_loss_with_uncertainty(
                anchors,
...
def _dense_box_regression_loss_with_uncertainty(
    ...
    loss_box_conf = F.binary_cross_entropy_with_logits(
        objectness_logits[fg_mask], objectness_targets_anchors, reduction="none"
    )
    ...
    storage = get_event_storage()
    storage.put_scalar(
        "rpn/conf_pos_anchors", torch.sigmoid(objectness_logits[fg_mask]).mean().item()
    )
    storage.put_scalar(
        "rpn/conf_neg_anchors", torch.sigmoid(objectness_logits[~fg_mask]).mean().item()
    )
